function courseDescr(userid, htmlId) {
  ...
  var model = {
    ...
    loadCourseData: function (subject, catalog) {
      ...
      var url = "https://api.uwaterloo.ca/v2/courses/" + subject + "/" + catalog + ".json?key=89660ae4dc888bbf1821c62d8eeb65a9";
      $.getJSON(url,
        function (d) {
        ...
        }).error(function() { throw "loadCourseData failed to load from " + url});
    }
  ...
  try {
  portal.loadTemplates("widgets/courseDescr/templates.txt",
    function (t) {
      templates = t;
